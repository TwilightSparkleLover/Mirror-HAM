 ...
 * Global Variables
 ********************************************************************************/
u8 g_SprObj[2];   // Global Sprite (index)
u8 g_NewFrame=0;
...
int main(void)
{
   ...
   g_SprObj[0] = ham_CreateObj((void *)&texture_Bitmap,  // A pointer to the tile data for this object
                                 0,                       // obj_shape
                                 3,                       // obj_size
   ...
   g_SprObj[1] = ham_CloneObj(g_SprObj[0],  // OBJ number of which you want to clone
                              240/2,        // X position of the cloned sprite (0..511)
                              160/2);       // Y position of the cloned sprite (0..255)
   ...
   while(1)
   {
       ...
       if(g_NewFrame)
       {
           ...
           ham_DrawText(2, 2, "Press A to change Z order");
           
           // Get joypad input
           if(F_CTRLINPUT_A_PRESSED)
           {
               // Change the Z Order of Sprites amongst each other
               ham_SetObjBefore(g_SprObj[0],    // OBJ number you want to move in front of another one
                                g_SprObj[1]);   // Target OBJ number our OBJ is to be set in front of
           }
           else
           {
               // Change the Z Order of Sprites amongst each other
               ham_SetObjBefore(g_SprObj[1],    // OBJ number you want to move in front of another one
                                g_SprObj[0]);   // Target OBJ number our OBJ is to be set in front of
           }
